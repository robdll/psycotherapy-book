 ...
 */
export async function tryFinalizeFromMercadoPagoPaymentId(
  paymentId: string,
  requestContext?: { clientIp: string | null; userAgent: string | null },
): Promise<{ ok: true } | { ok: false; reason: string; mpStatus?: string }> {
  ...
  return finalizeBookingAfterApprovedPayment({ bookingId, paymentId: id, requestContext });
}
